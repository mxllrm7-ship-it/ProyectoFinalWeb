...
export default function ServiceCard({ service }) {
  ...
  const proveedoresRestantes =
    (service.proveedores?.length || 0) - proveedoresVisibles.length;
        ...
        <div className="service-card-info">
          ...
          <div className="service-card-info-item">
            ...
            <span className="service-card-value">
              {service.proveedores?.length || 0} proveedore
              {service.proveedores?.length !== 1 ? "s" : ""} disponible
              {service.proveedores?.length !== 1 ? "s" : ""}
            </span>
          </div>
        </div>
        ...
        <div className="service-card-providers">
          {proveedoresVisibles.map((proveedor, index) => (
            <div key={index} className="service-card-provider">
              <div className="service-card-provider-name">
                {proveedor.nombre}
              </div>
          ...
          {proveedoresRestantes > 0 && (
            <div className="service-card-provider-more">
              +{proveedoresRestantes} proveedore
              {proveedoresRestantes !== 1 ? "s" : ""} más
            </div>
          )}
        </div>
      </div>
